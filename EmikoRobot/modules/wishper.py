...
async def stsrt(event):
    await event.reply(
        "**Heya, I am a Whisper Bot function for Lucky!**",
...
async def die(event):
    ...
    dn = event.builder.article(
        ...
        description="It's a whisper Bot function for Lucky!\n(c) Reeshuxd",
...
async def inline(event):
    ...
    try:
        ...
        await event.answer(
            [], switch_pm=f"@{me} [Username]|[Message]", switch_pm_param="whisper"
        )
    except ValueError:
        await event.answer(
            [], switch_pm="Give a message too!", switch_pm_param="whisper"
        )
    try:
        ui = await bot(us(user))
    except BaseException:
        await event.answer(
            [], switch_pm="Invalid User ID/Username", switch_pm_param="whisper"
        )
        return
...
**Note:** __Only {ui.user.first_name} can open this!__
    ...
    await event.answer(
        [dn], switch_pm="It's a secret message! Sssh", switch_pm_param="whisper"
    )
